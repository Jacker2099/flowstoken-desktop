...
async function fetchJson(fetcher, url, token) {
	...
	if (!response.ok) throw new Error(`GitHub release check failed (${response.status}): ${url}`);
...
async function publishedHostCapabilities(fetcher, repository, appVersion, token) {
	const tag = `v${appVersion}`;
	const release = await fetchJson(fetcher, `https://api.github.com/repos/${repository}/releases/tags/${tag}`, token);
	...
	for (const path of [
		...
		const source = await publishedSource(fetcher, repository, tag, path, token);
		if (source === null) continue;
		const version = /PLUGIN_API_VERSION\s*=\s*"(\d+\.\d+\.\d+)"/.exec(source)?.[1];
		if (!version) throw new Error(`Desktop ${tag} does not declare PLUGIN_API_VERSION in ${path}`);
		const schemaPath = "apps/desktop/src/main/abilities/open-marketplace/marketplace-schema.ts";
		const schema = await publishedSource(fetcher, repository, tag, schemaPath, token);
		...
		if (!schemaVersion || Number(schemaVersion) < 3 || !schema?.includes("z.literal(MARKETPLACE_SCHEMA_VERSION)")) {
			throw new Error(`Desktop ${tag} does not support marketplace schema v3`);
		}
		return version;
	}
	throw new Error(`Cannot find the Plugin API version in Desktop ${tag}`);
}
...
export async function verifyMarketplacePublication(manifest, { fetcher = fetch, appRepository = DEFAULT_APP_REPOSITORY, token } = {}) {
	...
	for (const { slug, release } of releases) {
		if (!hostVersions.has(release.minAppVersion)) {
			hostVersions.set(release.minAppVersion, await publishedHostCapabilities(fetcher, appRepository, release.minAppVersion, token));
		}
