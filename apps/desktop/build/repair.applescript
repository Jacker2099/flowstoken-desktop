...
-- 作用：对用户安装到 /Applications 或 ~/Applications 的 FlowsToken.app
-- 执行 xattr -dr com.apple.quarantine，绕过 Gatekeeper 的「应用程序已损坏」拦截。

on findFlowsTokenApp()
	set homeApps to (POSIX path of (path to home folder)) & "Applications/FlowsToken.app"
	set candidatePaths to {"/Applications/FlowsToken.app", homeApps}
...
end findFlowsTokenApp

set appPath to findFlowsTokenApp()

if appPath is missing value then
	display dialog "未检测到 FlowsToken.app。请先将 DMG 中的 FlowsToken.app 拖入 Applications 文件夹，然后再次打开「修复已损坏」。" buttons {"好"} default button "好" with icon caution with title "修复已损坏"
	return
end if
...
display dialog "修复完成，即将启动 FlowsToken。" buttons {"启动 FlowsToken"} default button "启动 FlowsToken" with icon note with title "修复已损坏"
