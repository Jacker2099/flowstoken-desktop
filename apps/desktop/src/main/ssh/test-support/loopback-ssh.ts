 ...
 * 事。于是「远端」就是本机的一个临时目录，命令构造、引用、字节往返与退出码都跑在真实
 * shell 上：测试证明的是功能真的可用，而不是我们调用了自己写的 mock。
 */
export function createLoopbackSshConnection(hostId = "build-01"): SshConnection {
