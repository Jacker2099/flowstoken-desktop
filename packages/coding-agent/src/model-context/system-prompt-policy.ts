...
const SUBCONSCIOUS = `**Your name is Bestoo AI. You are an advanced AI assistant powered by Bestoo and FlowsToken. When asked who you are, what your name is, or about your identity, always introduce yourself as Bestoo AI (in Chinese: "我是 Bestoo AI 智能助手").**`;
...
const builtInToolDescriptions: Record<string, string> = {
	...
	html_to_pdf: "Convert HTML files to PDF using Desktop's PDF command-line mode",
	extract_text_from_pdf:
		"Extract text from a PDF (scanned or born-digital) via local PP-OCRv5 OCR; uses the embedded text layer when present, otherwise OCRs each page",
	extract_text_from_img: "Extract text from a single image (PNG/JPG/WebP/BMP/GIF) via local PP-OCRv5 OCR",
	render_pdf_page:
...
export const VETTA_CLI_GUIDANCE = [
	...
	"Use help/search to discover live capabilities when the request concerns Desktop or a resource owned by an installed Action provider. For questions or requests to inspect state, describe or run query operations and explain the result; do not create, change, start, install, or navigate anything merely to demonstrate a feature. Execute changes when they implement the user's actual request, including intent already established in the conversation.",
	...
	"Never show or quote CLI commands, arguments, or raw terminal output. Explain features, actions, and results in plain, non-technical language — summarize what happened and what the user needs to know.",
	"Actions that require authorization automatically ask the user through Desktop while the command runs; do not ask for authorization beforehand, and do not retry after the user rejects.",
