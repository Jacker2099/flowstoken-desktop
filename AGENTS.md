...
- `apps/`：可交付的应用，依赖图的叶子节点，不被任何包 import。`desktop`、`cli-host`、`docs-site`、`mobile`（Expo/React Native）、`kotlin`（Kotlin Multiplatform，仅 Android）与 `im-gateway`（Go）。
...
```text
apps/*  (desktop / cli-host / docs-site / mobile / kotlin / im-gateway)
                      |
                      v
