...
非 Bun workspace 由独立的 path-filtered workflow 覆盖：`.github/workflows/im-gateway.yml` 对 Go Gateway 执行 tidy、vet、build、test、接口纪律和 golangci-lint；`.github/workflows/kotlin.yml` 对 Kotlin Mobile 执行 Android host tests 和 debug APK 构建。它们只在分支 push 或 PR 中对应目录或 workflow 自身变化时运行，不响应 tag push，也不把 Go/Kotlin 生命周期伪装成 JavaScript workspace 任务。
