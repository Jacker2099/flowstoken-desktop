...
3. **远端 helper**：见「远端交付」。已接入后台任务托管与文件变更通知；单次往返编辑的协议已具备，工具侧尚未使用。内置搜索没有做，搜索仍依赖远端的 ripgrep / fd。
...
- MCP 服务器运行在本机，无法访问远端项目：宿主不向它们通告工作区根，并在系统提示词中说明。插件按能力区分——声明的一次性命令（`command.run`）与文件能力按 cwd 归属改到远端执行，Git 面板因此直接作用于远端仓库；靠本机端口与界面通信的长驻进程（`command.spawn`，如开发服务器）在远程项目里明确报不支持。
