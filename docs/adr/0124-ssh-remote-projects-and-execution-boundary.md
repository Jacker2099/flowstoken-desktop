...
2. **SSH 可用形态**：`ssh-transport` 连接管理与主机模型；`SshFsBackend`；`runtime-ssh` 实现各工具 `*Operations` 端口；设置页与添加项目入口。
   ...
   搜索类工具（grep、glob、find、tree）在内部直接 spawn 本机 ripgrep，其进程启动点
   尚未抽成可注入端口。在抽出之前，远程会话**不注册**这四个工具——注册就意味着它们
   去搜本机磁盘并把结果当作远端项目的内容，正是本 ADR 要禁止的静默回退。远程会话
   改由模型在 bash 里用 grep/find，慢但结果来自正确的那台机器。
...
- **本机断开时当前回合中断**：前台与后台命令都绑定在 SSH 通道上，通道关闭时远端进程收到 SIGHUP 结束；会话记录留在本地，重连后可续接。这是本决策的结构性限制。让远端任务在断开后继续存活需要在远端托管进程并把输出写到远端文件，属于第二阶段 helper 的范围。
